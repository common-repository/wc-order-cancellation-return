...
class WC_Order_Cancellation_Return_Edit_Order_Cancellation_Approval {

	public function __construct() {
		add_action('woocommerce_admin_order_data_after_shipping_address', [$this, 'yoocr_wc_order_cancellation_return_edit_order_cancellation_approval'], 10, 1);
		add_action('admin_footer', [$this, 'yoocr_wc_order_cancellation_return_admin_footer_scripts']);
	}

	public function yoocr_wc_order_cancellation_return_edit_order_cancellation_approval($order) {
		if ($order->has_status('request-cancel')) {
			$cancellation_reason = $order->get_meta('request_cancellation_reason', true);

			echo '<p><strong>' . esc_html__('Cancellation requested', 'wc-order-cancellation-return') . '</strong><br>' . esc_html__('Reason:', 'wc-order-cancellation-return') . ' ' . esc_html($cancellation_reason) . '</p>';
			echo '<form method="post">';
			wp_nonce_field('approve_cancellation_action', 'approve_cancellation_nonce');
			echo '<input type="hidden" name="order_id" value="' . esc_attr($order->get_id()) . '">';
			echo '<button type="submit" name="approve_cancellation" class="button-secondary icon-button" title="' . esc_attr__('Approved', 'wc-order-cancellation-return') . '" style="margin-right: 5px;" onclick="return confirm(\'' . esc_js(__('Are you sure to approve this cancellation?', 'wc-order-cancellation-return')) . '\')"><span class="dashicons dashicons-yes"></span></button>';
			echo '</form>';
			
			echo '<input type="hidden" id="rejectionNonce" value="' . esc_attr(wp_create_nonce('handle_rejection_action')) . '">';
			echo '<button type="button" id="cancellation-rejected" class="button-secondary icon-button" title="' . esc_attr__('Rejected', 'wc-order-cancellation-return') . '" onclick="WC_Order_Cancellation_Return_Admin.openRejectionModal();"><span class="dashicons dashicons-no"></span></button>';
	...
	public function yoocr_wc_order_cancellation_return_admin_footer_scripts($is_request_cancel = false) {
		if ($is_request_cancel) {
			?>
			...
			var WC_Order_Cancellation_Return_Admin = {
				...
				submitRejectionReason: function(orderId) {
					...
					jQuery.post(ajaxurl, {
						action: "yoocr_wc_order_cancellation_return_handle_order_cancellation_rejection",
						order_id: orderId,
						reason: reason,
...
new WC_Order_Cancellation_Return_Edit_Order_Cancellation_Approval();
